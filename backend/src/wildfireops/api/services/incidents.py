...
class IncidentQueryService:
    ...
    async def list_incidents(self) -> tuple[IncidentSummaryResponse, ...]:
    ...
    async def get_incident(self, incident_id: str | UUID) -> IncidentDetailResponse:
        ...
        for snapshot in snapshots:
            ...
            frames.append(
                TimelineFrameResponse(
                    ...
                    detections=await self._detections(state),
                    exposed_asset_count=len(snapshot.asset_state),
                    freshness=_snapshot_freshness(
                        state,
                        last_observed_at,
                        self._settings.risk_fire_freshness_seconds,
    ...
    def _summary(self, snapshot: IncidentSnapshotModel) -> IncidentSummaryResponse:
        state = _mapping(snapshot.incident_state, "incident state")
        last_observed_at = _timestamp(state.get("last_observed_at"), "last observed")
...
def _incident_not_found(incident_id: str) -> ApiError:
    return ApiError(
        status_code=404,
...
def _risk(value: object) -> RiskResponse:
    ...
    for item in factors:
        ...
        contributions.append(
            RiskContributionResponse(
                ...
                raw_value=cast(JsonValue, factor.get("raw")),
                ...
                contribution=_number(
                    factor.get("contribution"), "risk contribution"
                ),
            )
        )
    return RiskResponse(
        ...
        algorithm_version=_string(
            risk.get("config_version"), "risk algorithm version"
        ),
        contributions=tuple(contributions),
    )


def _detailed_risk(value: object) -> DetailedRiskResponse:
    risk = _mapping(value, "risk")
    summary = _risk(value)
...
def _assets(value: object) -> tuple[ExposedAssetResponse, ...]:
    assets: list[ExposedAssetResponse] = []
        ...
        assets.append(
            ExposedAssetResponse(
                ...
                distance_meters=_number(
                    asset.get("distance_meters"), "asset distance"
                ),
...
def _simulated_resources(value: object) -> tuple[SimulatedResourceResponse, ...]:
    resources: list[SimulatedResourceResponse] = []
        ...
        resources.append(
            SimulatedResourceResponse(
                ...
                resource_type=_string(
                    resource.get("resource_type"), "resource type"
                ),
                capabilities=capabilities,
                capacity=_integer(resource.get("capacity"), "resource capacity"),
                available=_boolean(
                    resource.get("available"), "resource availability"
                ),
...
def _json_object(value: object, field: str) -> dict[str, JsonValue]:
    return cast(dict[str, JsonValue], dict(_mapping(value, field)))
